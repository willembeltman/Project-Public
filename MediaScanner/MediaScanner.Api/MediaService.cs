...
public class MediaService : IHostedService
{
    ...
    private readonly ConcurrentDictionary<string, ConcurrentDictionary<string, MediaFileDto>> _clientFiles = new(StringComparer.OrdinalIgnoreCase);
    ...
    private void IndexLocalStorage()
    {
        ...
        var localFiles = new ConcurrentDictionary<string, MediaFileDto>(StringComparer.OrdinalIgnoreCase);
    ...
    private void OnLocalFileChanged(string fullPath, FileUpdateType updateType)
    {
        ...
        var localDict = _clientFiles.GetOrAdd("Local", _ => new ConcurrentDictionary<string, MediaFileDto>(StringComparer.OrdinalIgnoreCase));
    ...
    private MediaFileDto? CreateLocalMediaFileDto(string fullPath)
    {
        try
        {
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                while (!token.IsCancellationRequested)
                {
                    ...
                    {
                        var files = JsonSerializer.Deserialize<List<MediaFileDto>>(msg.Payload ?? "[]") ?? new List<MediaFileDto>();
                        var dict = new ConcurrentDictionary<string, MediaFileDto>(StringComparer.OrdinalIgnoreCase);
                    ...
                    else if (msg.Type == MessageType.FileUpdate && currentClientId != null)
                    {
                        ...
                        {
                            var dict = _clientFiles.GetOrAdd(currentClientId, _ => new ConcurrentDictionary<string, MediaFileDto>(StringComparer.OrdinalIgnoreCase));
